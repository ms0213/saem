						...
						<tr>
							...
							<td width="20%" align="right">
								...
								<section id="search" class="alt">
									...
									name="searchForm" action="${pageContext.request.contextPath}/trade/list.do">
										<select name="condition" style="width:60%">
											...
											<option value="type"  ${condition=="type"?"selected='selected'":"" }>구분</option>
										</select>
					...
					<table class="table table-border table-list">
						...
						<c:forEach var="dto" items="${list}">
							<tr>
								...
								<td>${dto.listNum}</td>
								<td class="left">
									<span style="color:${dto.type=='완료'?'gray':'green'}">[${dto.type}] </span>
									<a href="${articleUrl}&num=${dto.num}" style="color:${dto.type=='완료'?'gray':''}; border-bottom:${dto.type=='완료'?'none':''}; ">
										${dto.subject}
										<c:if test="${listFile.size() > 0 }">
											<p><i class="far fa-file"></i></p>
										</c:if>
									</a>
								</td>		
								<td>
