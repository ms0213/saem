...
public class PhotoServlet extends MyUploadServlet {
	...
	protected void process(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(uri.indexOf("list.do") != -1) {
			...
			delete(req, resp);
		} else if (uri.indexOf("insertPhotoLike.do") != -1){
			insertPhotoLike(req, resp);
	...
	// AJAX - JSON
	private void insertPhotoLike(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		// 게시글 공감 저장
		PhotoDAO dao = new PhotoDAO();
		...
		try {
			...
			if(isNoLike.equals("true")) {
				dao.insertPhotoLike(num, info.getUserId()); // 좋아요 추가
			} else {
				dao.deletePhotoLike(num, info.getUserId()); // 좋아요 취소
			}
			photoLikeCount = dao.countPhotoLike(num); // 좋아요 갯수
		...
		job.put("state", state);
		job.put("photoLikeCount", photoLikeCount);
	...
	protected void listReply(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			forward(req, resp, "/WEB-INF/saem/gallery/listReply.jsp");
			return;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected void listReplyAnswer(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			forward(req, resp, "/WEB-INF/saem/gallery/listReplyAnswer.jsp");
			return;
		} catch (Exception e) {
			e.printStackTrace();
		}
